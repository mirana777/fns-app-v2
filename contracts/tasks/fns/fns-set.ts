...
import { getFNSRegistry, isFilDomain, labelhash, TLD_FIL_NAME, toNode, txParams } from "../../scripts/common";
...
task("fns-set-subnode", "Set subnode")
  ...
  .setAction(async ({ parent, owner, subnames }, hre) => {
    ...
    for (let name of subnames) {
      console.log(`Set subnode: ${name}.${parent}`);
      console.debug(`parent: ${parentNode}`);
      console.debug(`namehash: ${labelhash(name)}`);
