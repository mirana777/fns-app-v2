<template>
  <div v-loading="loading" class="p-2" element-loading-text="Submitting...">
    ...
    >
      ...
      <template v-if="domain.available && connected" #extra>
      ...
      <el-descriptions-item label="Expiration date" label-class-name="w-64">
        <div v-if="!domain.available" class="flex-center justify-start">
          ...
          <el-button
            v-if="!domain.available && connected"
            class="ml-2"
            size="small"
            type="primary"
            icon="Clock"
    ...
    <!-- Extend fee -->
    <template v-if="!domain.available && connected">
      <el-collapse v-model="activeNames" class="mt-8">
